...
int main()
{
    ...
    std::set<int> search_results = tree.reverse_radius_search(Eigen::Vector3d(2.1, 0, 0));
    for (auto it = search_results.begin(); it != search_results.end(); ++it)
    {
        std::cout << *it << std::endl;
    }

    tree.increase_vertex_radius(0, Eigen::Vector3d(0, 0, 0), 3);

    std::cout << "After adjusting radius" << std::endl;

    tree.printTree();

    std::set<int> search_results2 = tree.reverse_radius_search(Eigen::Vector3d(2.1, 0, 0));

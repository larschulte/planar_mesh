...
private:
    ...
    RRSReturnType node_reverse_radius_search(const std::shared_ptr<RRSNode>& node, const Eigen::Vector3d& point, std::vector<std::shared_ptr<Vertex>>& search_results, std::set<std::shared_ptr<Surface>, MeshObjectCompare>& intersected_surfaces);
...
public:
    ...
    RRSReturnType tree_reverse_radius_search(const Eigen::Vector3d& point, std::vector<std::shared_ptr<Vertex>> &search_results, std::set<std::shared_ptr<Surface>, MeshObjectCompare>& intersected_surfaces);
